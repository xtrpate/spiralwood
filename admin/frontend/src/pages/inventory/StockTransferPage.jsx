...
export default function StockTransferPage() {
  ...
  const submitTransfer = async (event) => {
    ...
    if (
      ...
      toast.error(
        "Select a product and enter a whole-number quantity for every row.",
      );
      return;
    }
  ...
  return (
    <div style={pageStyle}>
      ...
      <div style={noticeStyle}>
        All ready-made stock starts in the <strong>Warehouse</strong>. Move
        products to the <strong>Display Area</strong> before selling them
        through the POS.
      </div>
      ...
      <section style={sectionStyle}>
        <div style={sectionHeaderStyle}>
          <div>
            ...
            <p style={sectionText}>
              Choose where the stock is moving from and where it is going.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={submitTransfer}>
          <div style={transferRouteStyle}>
            ...
            <div style={routeArrowStyle} aria-hidden="true">
              →
            </div>
        ...
        <div style={filterGrid}>
          ...
          <label style={filterFieldStyle}>
            ...
            >
              ...
              {Object.entries(DIRECTIONS).map(([value, meta]) => (
      ...
      {(selected || detailsLoading) && (
        <div
          style={overlayStyle}
          onClick={() => !detailsLoading && setSelected(null)}
        >
            ...
            {detailsLoading && !selected ? (
              ...
              <>
                ...
                <div style={detailGrid}>
                  <Detail
                    label="From"
                    value={DIRECTIONS[selected.direction]?.from || "—"}
                  />
                  <Detail
                    label="To"
                    value={DIRECTIONS[selected.direction]?.to || "—"}
                  />
                  <Detail
                    label="User"
                    value={selected.transferred_by_name || "System"}
                  />
                    ...
                    <tbody>
                      {(selected.items || []).map((item) => (
                        <tr key={item.id}>
                          ...
                          <Td>{item.quantity}</Td>
                          <Td>
                            {item.warehouse_before} → {item.warehouse_after}
                          </Td>
                          <Td>
                            {item.display_before} → {item.display_after}
                          </Td>
...
function SummaryCard({ label, value }) {
  return (
    <div style={cardStyle}>
      ...
      <div style={cardValue}>{Number(value || 0)}</div>
    </div>
  );
}
...
function Detail({ label, value }) {
  return (
    <div>
      <div style={cardLabel}>{label}</div>
      <div style={{ marginTop: 4, fontSize: 12.5 }}>{value || "—"}</div>
    </div>
  );
}
...
const lineRow = {
  display: "grid",
  gridTemplateColumns: "minmax(280px,1fr) 150px auto",
  gap: 8,
  alignItems: "start",
};
const actionsStyle = {
