  ...
  const buildCabinetBox3D = useCallback(
    (options = {}) => {
      ...
      if (builderType === "base") {
        ...
        const outerWidth = cleanMm(options.width, 900, 300, 1200);
        const outerHeight = cleanMm(options.height, 876, 800, 950);
        const outerDepth = cleanMm(options.depth, 610, 500, 650);
      ...
      if (builderType === "wall") {
        ...
        const outerWidth = cleanMm(options.width, 900, 300, 1200);
        const outerHeight = cleanMm(options.height, 760, 300, 1100);
        const outerDepth = cleanMm(options.depth, 305, 250, 400);
        const thickness = cleanMm(options.thickness, 18, 12, 25);
        const backThickness = cleanMm(options.backThickness, 6, 3, 12);
        const bottomHeightFromFloor = Math.max(
          0,
          Number(Number(options.bottomHeightFromFloor ?? 0).toFixed(3)),
        );
        const shelfCount = Math.max(
          0,
          Math.min(8, Math.round(Number(options.shelfCount) || 0)),
      ...
      if (builderType === "tall") {
        ...
        const outerWidth = cleanMm(options.width, 600, 300, 900);
        const outerHeight = cleanMm(options.height, 2135, 2100, 2450);
        const outerDepth = cleanMm(options.depth, 610, 450, 650);
        const thickness = cleanMm(options.thickness, 18, 12, 25);
        const backThickness = cleanMm(options.backThickness, 6, 3, 12);
        const toeKickHeight = Math.max(
          0,
        ...
        const shelfCount = Math.max(
          0,
          Math.min(12, Math.round(Number(options.shelfCount) || 0)),
        );
        const dividerCount = Math.max(
          0,
          Math.min(4, Math.round(Number(options.dividerCount) || 0)),
        ...
        if (thickness * 2 >= outerWidth) {
          toast.error(
            "Panel thickness is too large for the selected tall cabinet width.",
          );
          return;
        }
        ...
        if (backThickness >= outerDepth) {
          toast.error(
            "Back thickness must be smaller than the tall cabinet depth.",
          );
          return;
        }

        const usableCarcassHeight =
          outerHeight - toeKickHeight - thickness * 2;

        if (usableCarcassHeight < 300) {
          toast.error(
            "Tall cabinet height is too small for the selected panel and toe-kick dimensions.",
          );
          return;
        }
        ...
        if (usableBayWidth < (dividerCount + 1) * 80) {
          toast.error(
            "Tall cabinet width is too small for the selected divider count and panel thickness.",
          );
          return;
        }
        ...
        if (outerHeight > floorY) {
          toast.error(
            "Tall cabinet is too high for the current Blueprint workspace. Lower the cabinet Height.",
          );
          return;
        }
        ...
        let buildNumber = 1;
        while (usedNames.has(`Tall Cabinet ${buildNumber}`)) {
          buildNumber += 1;
        }

        const groupId = makeGroupId();
        const groupLabel = `Tall Cabinet ${buildNumber}`;
