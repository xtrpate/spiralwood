...
const normalizeLegacyAutoName = (value = "") =>
  normalizeEstimateNameKey(value);
...
const mergeAutoItemsWithExistingPrices = (
  ...
  return autoItems.map((item) => {
    ...
    return normalizeItem({
      ...
      quantity:
        Number(match.quantity || 0) > 0 ? Number(match.quantity) : item.quantity,
    ...
    const loadData = async () => {
      try {
        ...
        if (estRes.data) {
          ...
          const latestBlueprintAutoItems = buildPreferredAutoItems(latestDesign);
          ...
          setItems(cleanedSavedItems.length ? cleanedSavedItems : [{ ...BLANK_ITEM }]);
