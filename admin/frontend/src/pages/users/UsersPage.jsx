...
const getPermissionDisplayName = (permission) => {
  ...
  const actionLabel =
    ...
    }[action] || humanizePermissionModule(action);
    ...
    <div className="wisdom-admin-users-v2">
      ...
      {permissionTarget && (
        <PermissionModal
          ...
          canEdit={isSuperAdminAccount(me) && permissionTarget?.id !== me?.id}
          onClose={() => {
            setPermissionTarget(null);
            setPermissionData(null);
          }}
...
function PermissionModal({ target, data, loading, canEdit, onClose, onSaved }) {
      ...
      >
        <div className="um-modal-header">
          <div>
            ...
            <p>
              {AUTHORITY_LEVELS[target?.authority_level || "user"]?.label ||
                "Standard"}
              {" · "}
              {getRoleLabel(target)}
            </p>
