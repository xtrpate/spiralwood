...
const formatUtcDateKeyForOperationsAppointmentReport = (date) =>
  [
...
const shiftOperationsAppointmentDateKey = (dateKey, days) => {
  const [year, month, day] = String(dateKey).split("-").map(Number);
  return formatUtcDateKeyForOperationsAppointmentReport(
...
const buildOperationsAppointmentDateRange = ({
  ...
  if (!OPERATIONS_APPOINTMENT_DATE_FILTERS.has(normalizedFilter)) {
    const error = new Error("Invalid operations appointment date filter.");
    error.status = 400;
    throw error;
  }
  ...
  if (normalizedFilter === "custom") {
    ...
    if (fromKey && !datePattern.test(fromKey)) {
      const error = new Error("Start date must use YYYY-MM-DD.");
      error.status = 400;
      throw error;
    }

    if (toKey && !datePattern.test(toKey)) {
    ...
    return {
      ...
      endKey: toKey ? shiftOperationsAppointmentDateKey(toKey, 1) : null,
    };
  }
  ...
  let endKey = shiftOperationsAppointmentDateKey(todayKey, 1);

  if (normalizedFilter === "yesterday") {
    startKey = shiftOperationsAppointmentDateKey(todayKey, -1);
    endKey = todayKey;
  } else if (normalizedFilter === "this_week") {
    // Preserve the Operations Report's existing Sunday-Saturday week.
    const dayOfWeek = new Date(Date.UTC(year, month - 1, day)).getUTCDay();
    startKey = shiftOperationsAppointmentDateKey(todayKey, -dayOfWeek);
    endKey = shiftOperationsAppointmentDateKey(startKey, 7);
    ...
    endKey = formatUtcDateKeyForOperationsAppointmentReport(
...
const getOperationsAppointmentReport = async (req, res) => {
  try {
    ...
    if (req.user.role === "staff") {
      where.push("a.assigned_staff_id = ?");
    ...
    // Appointment schedule values are Manila business wall-clock datetimes.
    const reportDateSql =
      "COALESCE(a.scheduled_date, a.preferred_date, a.updated_at)";

    if (startKey) {
      where.push(`${reportDateSql} >= ?`);
      params.push(`${startKey} 00:00:00`);
    }

    if (endKey) {
      where.push(`${reportDateSql} < ?`);
      params.push(`${endKey} 00:00:00`);
    }
    ...
    if (includeSummary) {
      const [[summaryRow]] = await db.query(
        `SELECT
           ...
           COALESCE(SUM(
             CASE
               WHEN LOWER(COALESCE(a.status, '')) IN (
                 'pending',
                 'scheduled',
                 'in_progress'
               )
    ...
    console.error("[pos.schedule GET /appointments operations report]", err);
    return res.status(500).json({
      message: "Failed to load appointments report.",
    });
  }
};
...
exports.getAppointments = async (req, res) => {
  if (String(req.query.operations_report || "").trim() === "1") {
    return getOperationsAppointmentReport(req, res);
  }
